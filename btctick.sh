...
then
	...
	cap=`cat coinmarketcap_data_$date\.html | grep -o -P 'Cap":.{0,500}' | head -n 2 | tail -n 1 | cut -d":" -f 2 | cut -d"," -f 1`
	#echo $cap
	...
	echo "----------"
	echo
	echo "You are at btctick master branch edit 2026-01-13 - default FIAT is USD (\$)."
