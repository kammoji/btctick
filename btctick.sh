...
then
	...
	cap=`cat coinmarketcap_data_$date\.html | grep -o -P 'Cap":.{0,500}' | head -n 15 | tail -n 1 | cut -d":" -f 21 | cut -d"," -f 1`
	#echo $cap
	#cap_parsed=`printf "%.0f" $cap | sed ':a;s/\B[0-9]\{3\}\>/,&/;ta'`
	price=`cat coinmarketcap_data_$date\.html | grep -o -P 'price".{0,500}' | head -n 15 | tail -n 1 | cut -d":" -f 2 | cut -d"." -f 1`
	...
	echo "----------"
	echo
	echo "You are at btctick master branch edit 2026-04-13 - default FIAT is USD (\$)."
	echo "btctick is public domain with NO WARRANTY. Use at your own discretion."
	echo
	echo "NEWS: Happy "$'\u20bf'" bear year 2026! Start preparing for the next bull run in ca. 3 years, it will be sooner than you think!"
	echo
	date
...
#        	exit 1
	...
	echo
	echo "ERROR: no data link to coinmarketcap.com"
